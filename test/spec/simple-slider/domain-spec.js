describe('SimpleSlider slider axis', () => {
  ...
  it('should be created', () => {
    let x_axis = d3.select('#simple-slider-chart .d3-chart-domain');
    expect(x_axis).not.toBeNull();
  });

  it('should be placed with a padding to the top', () => {
    let domainTransform = d3.transform(d3.select('#simple-slider-chart .d3-chart-domain').attr('transform'));
    expect(domainTransform.translate[1]).toBe(30);
  });

  it('should have specified amount of ticks', () => {
    let ticks = d3.select('#simple-slider-chart .d3-chart-domain').selectAll('g.tick').size();
  ...
  it('should have specified tick description', () => {
    ...
    d3.select('#simple-slider-chart .d3-chart-domain').selectAll('g.tick').each(function(d, i) {
